    ...
    <div className="mobile-frame">
      ...
      <div style={{ height: 660, background: 'linear-gradient(135deg,#e8f4f8 0%,#d4edda 50%,#fff3cd 100%)', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: 287, right: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div onClick={() => { addRecentAction('Locate in rental cost'); info('地图', '定位 (demo)'); }} style={{ background: '#fff', padding: 14, borderRadius: '50%', boxShadow: '0 4px 40px rgba(0,0,0,0.1)', cursor: 'pointer' }}>🎯</div>
          <div onClick={() => { addRecentAction('Route from rental cost'); onNavigate?.('map-route'); }} style={{ background: '#fff', padding: 14, borderRadius: '50%', boxShadow: '0 4px 40px rgba(0,0,0,0.1)', cursor: 'pointer' }}>✅</div>
        </div>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderRadius: '24px 24px 0 0', padding: '28px 24px 40px', boxShadow: '0 -4px 40px rgba(0,0,0,0.1)' }}>
          ...
          <button onClick={() => { addRecentAction('Select payment from rental cost'); onNavigate?.('payment-select'); }} style={{ marginTop: 20, width: '100%', background: '#fecc2a', border: 'none', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>
