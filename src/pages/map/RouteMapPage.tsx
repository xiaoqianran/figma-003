    ...
    <div className="mobile-frame">
      ...
      <div style={{ height: 660, background: 'linear-gradient(135deg,#e8f4f8 0%,#d4edda 50%,#fff3cd 100%)', position: 'relative' }}>
        ...
        {Array.from({length:6}).map((_,i) => (
          <div key={i} style={{ position: 'absolute', fontSize: 15, top: 150 + (i*38), left: 40 + (i*35) + (i%2 ? 60 : 0), transform: `rotate(${i*20-40}deg)` }} onClick={() => { addRecentAction('Car selected on route map'); info('车辆', '车辆已选 (demo)'); }}>🚕</div>
        ))}

        <div style={{ position: 'absolute', top: 340, left: 69, fontSize: 24 }} onClick={() => { addRecentAction('Destination picked on route'); success('目的地', '目的地已选 (demo)'); }}>📍</div>
