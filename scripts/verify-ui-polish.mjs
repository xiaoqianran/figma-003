...
// --- 6. Pages Chinese markers (full Latin inventory is scripts/scan-pages-zh.mjs via npm run test:ui) ---
...
assert(read('src/pages/core/Search1Page.tsx').includes('空格') && read('src/pages/core/Search1Page.tsx').includes('（演示）'), 'Search1 Chinese keys + 演示 suffix')
assert(!read('src/pages/core/Search1Page.tsx').includes('(demo)'), 'Search1 has no Latin (demo) suffix')
assert(!read('src/pages/core/Search2Page.tsx').includes('(demo)'), 'Search2 has no Latin (demo) suffix')
assert(fs.existsSync(path.join(root, 'scripts/scan-pages-zh.mjs')), 'scan-pages-zh.mjs present (wired by npm run test:ui)')
